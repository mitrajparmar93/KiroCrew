...
describe('VariablesPanel', () => {
  ...
  describe('global scope', () => {
    ...
    it('clears a stale row draft when a bulk apply lands', async () => {
      // A bulk apply replaces the whole scope, so a half-typed row draft describes a
      // value that no longer exists. Left in place, the next blur commits it back over
      // what the paste just set -- the operator's own bulk edit silently undone.
      variables.mockResolvedValue(view({ global: { REGION: 'eu-west-1' } }))
      mount()
      await ready()

      fireEvent.change(screen.getByDisplayValue('eu-west-1'), { target: { value: 'stale' } })
      fireEvent.click(screen.getByRole('button', { name: /bulk edit/i }))
      const box = await screen.findByRole('textbox', { name: /NAME=value per line/i })
      fireEvent.change(box, { target: { value: 'REGION=us-east-1' } })
    ...
    it('keeps row drafts when a bulk apply is refused', async () => {
      // The other direction: clearing on failure would discard what the user typed
      // while the scope still holds the value that draft was editing.
      variables.mockResolvedValue(view({ global: { REGION: 'eu-west-1' } }))
      saveVariables.mockRejectedValue(new Error('boom'))
      mount()
      await ready()
