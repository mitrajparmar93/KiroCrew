...
function VariableTable({
  ...
  const applyBulk = () => {
    ...
    onSave({ bulk: bulkText, base: bulkBase }, () => {
      setBulkOpen(false)
      // Row drafts are cleared too, and only on SUCCESS.
      //
      // A bulk apply replaces the whole scope, so any half-typed row draft describes a
      // value that no longer exists. Left in place, the next blur commits it back over
      // what the bulk text just set -- the operator's own paste silently undone by a
      // row they had touched before opening the editor.
      //
      // Tied to success for the same reason the row-save and delete paths are: clearing
      // on failure would discard what the user typed while the scope still holds the
      // value that draft was editing.
      setDrafts({})
    })
  }
