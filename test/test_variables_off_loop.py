...
class TestResolutionIsOffLoop:
    ...
    def test_the_offload_forms_are_actually_present(self):
        ...
        assert "asyncio.to_thread( render_nudge_message" in " ".join(nudge.split())
        # Normalised, because the call is now multi-line: it returns the unresolved
        # names so the caller can append them ON the loop -- `slot.append` ends in
        # `asyncio.Event.set()`, which is not thread-safe.
        assert "asyncio.to_thread( _expand_message_variables" in " ".join(runner.split())
