...
class TestAValueCannotIntroduceATrigger:
    """A variable's VALUE must never turn a plain message into a prompt/skill trigger.

    On the normal pass the ordering already prevents it -- `@prompt` and `$skill`
    resolve BEFORE expansion, which the ordering ratchets in this file pin. But that
    ordering only holds for text arriving as the operator typed it, and a turn that
    fails before streaming is re-queued with the message as it then stood: already
    expanded. On the replay, a value of `@ops` reaches the resolver as a leading `@`
    and loads a prompt file the operator never named.

    Guarded by COMPARING the two texts at the one place that has both, rather than by
    trying to detect which pass this is.
    """

    def test_a_value_that_adds_a_leading_at_is_refused(self):
        assert runner_mod._introduces_a_trigger("{{who}} review this", "@ops review this")

    def test_a_value_that_adds_a_leading_dollar_is_refused(self):
        assert runner_mod._introduces_a_trigger("{{s}} go", "$deploy go")

    def test_the_operators_own_trigger_still_works(self):
        """Refusing this would break the ordinary case the feature exists to serve."""
        assert not runner_mod._introduces_a_trigger("@ops {{task}}", "@ops ship it")

    def test_a_value_in_the_middle_is_untouched(self):
        """Only a LEADING trigger fires the resolvers, so only that is refused."""
        assert not runner_mod._introduces_a_trigger("ping {{who}}", "ping @ops")

    def test_leading_whitespace_does_not_hide_it(self):
        """The resolvers strip before matching, so this check must too."""
        assert runner_mod._introduces_a_trigger("{{who}} x", "   @ops x")

    def test_the_pipeline_restores_the_authored_text(self):
        """Structural: the guard must put the ORIGINAL back, not blank the message or
        merely log. Leaving the expanded text in place is the defect."""
        region = inspect.getsource(runner_mod._run_chat)
        assert "_introduces_a_trigger(" in region
        assert "message = _authored_before_expansion" in region
